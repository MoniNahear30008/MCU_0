...
#define BCM_TASK_STATE_RUNNING   (0UL)  /**< @brief Task is in running state    */
#define BCM_TASK_STATE_WAITING   (1UL)  /**< @brief Task is in waiting state     */
#define BCM_TASK_STATE_READY     (2UL)  /**< @brief Task is in ready state      */
#define BCM_TASK_STATE_SUSPENDED (3UL)  /**< @brief Task is in suspended state  */
/** @} */

/** @brief Wait event wrapper function

    This function takes event mask and waits for one of those events.
    ...
    @retVal      BCM_ERR_OK           BCM_WaitEvent executed successfully
    @retVal      BCM_ERR_UNKNOWN      Integration fault
    @retVal      BCM_ERR_NOPERM       Calling task is not an extended task
    @retVal      BCM_ERR_BUSY         Calling task occupies resources
    @retVal      BCM_ERR_INVAL_STATE  Function called at interrupt level

    @post On successfully executing BCM_WaitEvent, one or more events are set
...
/** @brief Set event wrapper function
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            BCM_SetEvent executed successfully
    @retVal      BCM_ERR_UNKNOWN       Integration fault
    @retVal      BCM_ERR_NOPERM        referenced task is not an extended task
    @retVal      BCM_ERR_INVAL_STATE   The referenced task is in suspended state
    @retVal      BCM_ERR_INVAL_PARAMS  Input task is invalid

    @post On successfully executing BCM_SetEvent, the events of given task are set according to the event mask.
...
/** @brief Get event wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            BCM_GetEvent executed successfully
    @retVal      BCM_ERR_UNKNOWN       Integration fault
    @retVal      BCM_ERR_NOPERM        Referenced task is not an extended task
    @retVal      BCM_ERR_INVAL_STATE   The referenced task is in suspended state
    @retVal      BCM_ERR_INVAL_PARAMS  Input task is invalid

    @post On successfully executing BCM_GetEvent, returns the current state of all event bits of the input task
...
/** @brief Clear event wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            BCM_ClearEvent executed successfully
    @retVal      BCM_ERR_UNKNOWN       Integration fault
    @retVal      BCM_ERR_NOPERM        Call is not from an extended task
    @retVal      BCM_ERR_INVAL_STATE   Function called at interrupt level

    @post On successfully executing BCM_ClearEvent, the events of the task calling BCM_ClearEvent are cleared
...
/** @brief BCM_ActivateTask wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK             BCM_ActivateTask executed successfully
    @retVal      BCM_ERR_UNKNOWN        Integration fault
    @retVal      BCM_ERR_MAX_ATTEMPS    Too many activations of given task
    @retVal      BCM_ERR_INVAL_PARAMS   Input task is invalid

    @post On successfully executing BCM_ActivateTask, the task is transferred from the suspended state into the ready state.
...
/** @brief BCM_TerminateTask wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK              BCM_TerminateTask terminated successfully
    @retVal      BCM_ERR_UNKNOWN         Never reaches this upon successful termination
    @retVal      BCM_ERR_UNKNOWN         Integration fault
...
/** @brief StartOS wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal BCM_ERR_OK       BCM_StartOS successful
    @retVal BCM_ERR_UNKNOWN  Integration fault
...
/** @brief BCM_SetRelAlarm wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            Alarm set successfully
    @retVal      BCM_ERR_UNKNOWN       Integration fault
    @retVal      BCM_ERR_INVAL_STATE   Alarm already in use
    @retVal      BCM_ERR_INVAL_PARAMS  aIncrement is 0UL
    @retVal      BCM_ERR_INVAL_PARAMS  aIncrement is greater than counter limit
    @retVal      BCM_ERR_INVAL_PARAMS  aAlarmID is invalid
...
/** @brief BCM_CancelAlarm wrapper
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            Alarm cancel successfully
    @retVal      BCM_ERR_UNKNOWN       Integration fault
    @retVal      BCM_ERR_INVAL_STATE   Alarm already in use
...
/** @brief Handover execution from ROM to bootloader or bootloader to firmware.
    ...
    @limitations This function does not return unless there is an integration fault
...
/** @brief Increment the counter
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            Counter incremented successfully
    @retVal      BCM_ERR_INVAL_PARAMS  aCounter is invalid
...
/** @brief Get the current task ID
    ...
    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            Task id successfully returned
    @retVal      BCM_ERR_INVAL_PARAMS  aCurTaskID is NULL
...
/** @brief Get the task state
    ...
    @pre Task should be called from within task context.

    @param[in]   aTaskId               Task ID
    @param[in]   aTaskState            Pointer to store the task state

    Return values are documented in reverse-chronological order
    @retVal      BCM_ERR_OK            Task state successfully returned
    @retVal      BCM_ERR_INVAL_PARAMS  aTaskState is NULL
    @retVal      BCM_ERR_INVAL_PARAMS  aTaskId is invalid task
...
/**
    ...
    @retval         #BCM_ERR_INVAL_PARAMS     aCompList is NULL
    @retval         #BCM_ERR_INVAL_PARAMS     aData is NULL
    @retval         #BCM_ERR_INVAL_PARAMS     aMsgId is 0ULL
    ...
    @note Debug messages are posted using this API. So adding debug logs in this API will create
          recursive calls. So debug prints should not be added in this API.
*/
